...
-- Save Function
local function SaveSeason2Position()
    local DungeonFrame = PVEFrame
    if DungeonFrame then
        local frameScale = Season2:GetEffectiveScale()
        local parentScale = DungeonFrame:GetEffectiveScale()

        local season2Left = Season2:GetLeft() * frameScale
        local season2Right = Season2:GetRight() * frameScale
        local season2Bottom = Season2:GetBottom() * frameScale
        ...
        local anchorToRight = (season2Left >= dungeonRight - 10)

        local offsetX, anchor
        if anchorToRight then
            offsetX = (season2Right - dungeonRight) / parentScale
            anchor = "BOTTOMRIGHT"
        else
            offsetX = (season2Left - DungeonLeft) / parentScale
            anchor = "BOTTOMLEFT"
        end
        local offsetY = (season2Bottom - dungeonBottom) / parentScale

        MDungeonTeleports.Season2Pos = {x = offsetX, y = offsetY, anchor = anchor}
    end
end

-- Load Function
local function LoadandReattachSeason2Position()
    local DungeonFrame = PVEFrame
    if DungeonFrame and MDungeonTeleports.Season2Pos then
        local pos = MDungeonTeleports.Season2Pos
        ...
        Season2:ClearAllPoints()
        local anchor = pos.anchor or "BOTTOMRIGHT"
        Season2:SetPoint(anchor, DungeonFrame, anchor, adjustedX, adjustedY)
    end
end

-- MARK: Login Start Up
local function OnPlayerLogin()
    LoadandReattachSeason2Position()
    MDTUpdateSeason2Colors()
    MDTUpdateSeason2Border()
    MDTUpdateFontS2()
    MDTCompactModeSeason2()
    MDTShiftDragToggleS2()
end
...
function MDTSeason2_UpdateVisibility(show)
    local seasonFrame = _G["Season2"]
    if seasonFrame then
...
-- MARK: Season 2 Frame
local Season2 = CreateFrame("Frame", "Season2", PVEFrame)
Season2:SetSize(164, 130)
Season2:SetMovable(true)
Season2:EnableMouse(true)
Season2:RegisterForDrag("LeftButton")
Season2:SetClampedToScreen(true)

Season2.isBeingDragged = false
...
-- Season 2 Backdrop and Borders
local Season2Backdrop = Season2:CreateTexture(nil, "BACKGROUND")
Season2Backdrop:SetAllPoints(Season2)
Season2Backdrop:SetColorTexture(
...
local borderTop = Season2:CreateTexture(nil, "OVERLAY")
...
borderTop:SetPoint("TOPLEFT", Season2, "TOPLEFT", borderThickness, 0)
borderTop:SetPoint("TOPRIGHT", Season2, "TOPRIGHT", -borderThickness, 0)
borderTop:SetHeight(borderThickness)

local borderBottom = Season2:CreateTexture(nil, "OVERLAY")
...
borderBottom:SetPoint("BOTTOMLEFT", Season2, "BOTTOMLEFT", borderThickness, 0)
borderBottom:SetPoint("BOTTOMRIGHT", Season2, "BOTTOMRIGHT", -borderThickness, 0)
borderBottom:SetHeight(borderThickness)

local borderLeft = Season2:CreateTexture(nil, "OVERLAY")
...
borderLeft:SetPoint("TOPLEFT", Season2, "TOPLEFT", 0, 0)
borderLeft:SetPoint("BOTTOMLEFT", Season2, "BOTTOMLEFT", 0, 0)
borderLeft:SetWidth(borderThickness)

local borderRight = Season2:CreateTexture(nil, "OVERLAY")
...
borderRight:SetPoint("TOPRIGHT", Season2, "TOPRIGHT", 0, 0)
borderRight:SetPoint("BOTTOMRIGHT", Season2, "BOTTOMRIGHT", 0, 0)
borderRight:SetWidth(borderThickness)

-- Season 2 Title
local Season2Title = Season2:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
Season2Title:SetPoint("TOP", Season2, "TOP", 0, -10)
Season2Title:SetFont(MDungeonTeleports.customFont, 13, "OUTLINE")
Season2Title:SetText(L["TITLE_S2"])
...
function MDTUpdateS2ButtonTexture()
...
for i, data in ipairs(MDT_Season2Row1) do
    data.index = i
    data.row = 1
    CreateSpellButton(Season2, data, rowOffset)
end

for i, data in ipairs(MDT_Season2Row2) do
    data.index = i
    data.row = 2
    CreateSpellButton(Season2, data, row2Offset)
end

-- Compact Mode Start
function MDTCompactModeSeason2()
    if MDungeonTeleports.CompactMode == true then
        CompactModeSeason2On()
    else
        CompactModeSeason2Off()
    end
end

-- MARK: Compact Mode On
function CompactModeSeason2On()
    Season2:SetSize(142, 94)
    Season2Title:SetPoint("TOP", Season2, "TOP", 0, -4)
    Season2Title:SetFont(MDungeonTeleports.customFont, 11, "OUTLINE")

    local buttonSpacing = 2
    local compactRowOffset = -18
    local compactRow2Offset = -57

    local numColumns = #MDT_Season2Row1
...
function CompactModeSeason2Off()
    Season2:SetSize(164, 130)
    Season2Title:SetPoint("TOP", Season2, "TOP", 0, -10)
    Season2Title:SetFont(MDungeonTeleports.customFont, 13, "OUTLINE")
...
function MDTShiftDragToggleS2()
    if MDungeonTeleports.ShiftDragToggle == true then
        Season2:SetScript("OnDragStart", function(self)
...
function MDTUpdateSeason2Colors()
    ...
    if Season2Backdrop then
        Season2Backdrop:SetColorTexture(bg.r, bg.g, bg.b, bg.a)
    end
...
function MDTUpdateSeason2Border()
...
-- MARK: Font Update
function MDTUpdateFontS2()
    local customFont = MDungeonTeleports.customFont
    Season2Title:SetFont(customFont, 11, "OUTLINE")

    local tx2 = MDungeonTeleports.titleColor

    Season2Title:SetTextColor(tx2.r, tx2.g, tx2.b)
