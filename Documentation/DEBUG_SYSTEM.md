# NextKey Debug System - Complete Developer Guide
...
This document consolidates all debug system documentation for developers, including guidelines, API reference, category usage, and best practices.
...
10. [Category Groups](#category-groups)
...
The debug system organizes categories into 5 logical groups:

#### Core Systems
- **Purpose**: Fundamental addon functionality and initialization
- Categories: startup, events, performance, database, config, options

#### Communications
- **Purpose**: Network and addon-to-addon communication
- Categories: communications, comms, libopenraid, raiderio, blizzard

#### Features & UI
- **Purpose**: User-facing features and interface elements
- Categories: ui, components, tooltip, teleport, lootwindow, profiles

#### Data Processing
- **Purpose**: Data calculation and processing systems
- Categories: keystones, season, IOCalculator, ioc, fakeplayerservice
...
```
Input Message → Check Level → DEV_MODE? → Check Category → Output
     ↓              ↓            ↓           ↓           ↓
  Message +    ERROR/USER/    True/False  Enabled/   Formatted
  Category     DEV/TRACE      (Debug?)   Disabled?   Output
```
